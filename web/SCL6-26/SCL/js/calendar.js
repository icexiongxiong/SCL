(function(w){
	w.Calendar = function (opt) {
		...
		content.addEventListener(evt.type,function (event) {
		    if(event.target.tagName=="DIV" && event.target.nodeType=="1" && hasclass(event.target.className,"canChoose")){
				...
				var reg=/[\u4E00-\u9FA5]/g;
				var result=oyear.innerHTML.replace(reg,'');
				var year = result;
				var result1=omonth.innerHTML.replace(reg,'');
				var month = result1;
	...
	// 初始化日期显示方法
	function setContent(el, fistWeek, monDaynum, nowDay,dateObj){
		// 留空
		...
		for(var i=1;i <= monDaynum;i++){
			...
			var year1=getYear(dateObj);
			var month1=getMonth(dateObj);
//			<%
//			if(tdList!=null){
...
//					
//		
//			if(i==<%tdList.get(i).getday;%>&&year1==<%tdList.get(i).getmonth;%>&&month1==<%tdList.get(i).getyear;%>){  
//				
//			
