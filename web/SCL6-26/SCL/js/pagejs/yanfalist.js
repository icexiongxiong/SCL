...
var limit = 10;
var start = 0;
var yanfaapp = angular.module('yanfa', ['CommonApp']);
yanfaapp.controller('yanfacontroller', function($scope, $http, URLParam) {
	leader = $.cookie("leader");
	organizationname = $.cookie("organizationname");
	var data = 0;
	if(organizationname == "中心主任" && leader == 'true') {
		data = 1;
	} else if(leader == 'true') {
		data = 2;
	}
	...
	$(function() {
		...
		$http({
			...
			if(response.success == true) {
				//      $scope.yanfaroots = response.root; 
	...
	$scope.editkh = function(id) {
		window.location.href = "edityanfa.html?id=" + id;

	}

	$scope.delkh = function(id) {
		$.ajax({
			url: yh + "/companyresearch/delete/" + id,
			xhrFields: {
				withCredentials: true
			},
