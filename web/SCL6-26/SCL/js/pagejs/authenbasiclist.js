...
var start = 0;
var pageData;
var authbasiclistapp = angular.module('authbasiclist', ['CommonApp']);
authbasiclistapp.controller('authbasiclistcontroller', function($scope, $http, URLParam) {
	...
	$(function() {
		...
		$http({
			...
			if(response.success == true) {
				...
				console.log(response.root[0].username);
			}
		});
	...
	$scope.addxq = function(id, companyname) {
		window.location.href = "addauthendetail.html?id=" + id + "&name=" + companyname;

	}

	$scope.editkh = function(id) {
		window.location.href = "editbasicauthen.html?id=" + id;
